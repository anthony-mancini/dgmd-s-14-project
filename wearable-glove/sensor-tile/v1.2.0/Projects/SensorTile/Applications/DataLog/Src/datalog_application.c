...
void RTC_Handler( RTC_HandleTypeDef *RtcHandle )
{
  ...
  {
	// Column 1, Time Stamp output
    sprintf( dataOut, "time:%02dH%02dM%02dS%02dSS,", stimestructure.Hours, stimestructure.Minutes, stimestructure.Seconds, subSec );
...
void Accelero_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
      // Columns 2-4, Accelerometer output
      sprintf( dataOut, "accX:%d,accY:%d,accZ:%d,", (int)acceleration.AXIS_X, (int)acceleration.AXIS_Y, (int)acceleration.AXIS_Z );
      CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));   
...
void Gyro_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
      // Columns 5-7, Gyroscope output
      sprintf( dataOut, "gyrX:%d,gyrY:%d,gyrZ:%d,", (int)angular_velocity.AXIS_X, (int)angular_velocity.AXIS_Y, (int)angular_velocity.AXIS_Z );
      CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));
...
void Magneto_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
      // Columns 8-10, Magneto output
      sprintf( dataOut, "magX:%d,magY:%d,magZ:%d", (int)magnetic_field.AXIS_X, (int)magnetic_field.AXIS_Y, (int)magnetic_field.AXIS_Z );
      CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ));
